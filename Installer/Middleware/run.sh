...
CONFIG_PATH="../../Configuration/Installer/Middleware/middleware.yaml"
...
# yamale -s ../../Configuration/Installer/Middleware/schema.yaml $CONFIG_PATH || {
...
if [[ "$ACTION" == "plan" ]]; then
  terraform plan -var="environment=$ENV_NAME" \
    -var="middleware_config_path=$CONFIG_PATH" \
