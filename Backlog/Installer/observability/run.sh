...
CONFIG_PATH="../../Configuration/Installer/Observability/observability.yaml"
BACKEND_TYPE="${4:-local}"
WORKSPACE="${ENV_NAME}-observability"
...
# yamale -s ../../Configuration/Installer/Observability/schema.yaml $CONFIG_PATH || {
...
if [[ "$ACTION" == "plan" ]]; then
  terraform plan -var="environment=$ENV_NAME" \
    -var="observability_config_path=$CONFIG_PATH" \
