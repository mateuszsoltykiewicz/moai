...
from .api import router as heating_router
...
async def lifespan(app: FastAPI):
    logger.info("Starting HeatingJob...")
    setup_metrics()
    setup_tracing(service_name="HeatingJob")
    await HeatingJobManager.setup()
    logger.info("HeatingJob started successfully")
    yield
    logger.info("Shutting down HeatingJob...")
    await HeatingJobManager.shutdown()
    logger.info("HeatingJob shutdown complete")
